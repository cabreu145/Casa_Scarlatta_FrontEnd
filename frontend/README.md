...
## Integraci�n Auth API (Frontend)

Este frontend soporta dos modos para autenticaci�n:
...
No se conectaron a�n clases, reservas ni waitlist.


### Registro extendido (Auth API)
Cuando `VITE_USE_API_AUTH=true`, registro env�a:
- requeridos: `email`, `name`, `password`
- opcionales: `phone`, `birth_date`, `gender`

El adapter normaliza g�nero de UI a valores backend:
`femenino | masculino | otro | prefiero_no_decir`.

## Estado de cierre Auth
- Integraci�n Auth API validada manualmente end-to-end.
...
Siguiente m�dulo recomendado:
- Clases lectura con feature flag `VITE_USE_API_CLASSES`.

## Integraci�n Clases Lectura (Feature Flag)
...
- Admin > Clases en API mode tambi�n usa `GET /api/v1/clases?page=&page_size=&search=&discipline=&status=&coach_id=`, `POST /api/v1/clases`, `PUT /api/v1/clases/{id}` y `DELETE /api/v1/clases/{id}`.
- Admin > Coaches en API mode usa `GET/POST/PUT/PATCH/DELETE /api/v1/coaches`, `POST /api/v1/coaches/{id}/avatar`, `GET /api/v1/coaches/public`, y `coachesStore` queda solo como fallback legacy cuando flags API est�n apagados.
...
- En API mode, `Fecha espec�fica` y `Programar publicaci�n` quedan bloqueados hasta que exista contrato de ocurrencias/publicaci�n.
...
- `duration_minutes` es alias preferido en frontend; backend tambi�n acepta `duration_min`.
- `description` se acepta en contrato, pero backend a�n no la persiste.
- No se integraron a�n reservas ni waitlist.

## Validaci�n manual (backend real)
- Auth API y Clases lectura API validadas con flags activos.
- Backend clases no env�a `coach_name`; frontend muestra fallback (`Coach #id`) sin romper UI.
- Modo fallback sigue disponible con flags en `false`.

Siguiente integraci�n recomendada: Reservas API.

## Integraci�n Reservas API (Feature Flag)
...
## Integraci�n Cortes de Caja (API mode)
...
- Exportaci�n CSV usa filename de `Content-Disposition` y fallback `finanzas-<type>-<from>_<to>.csv`.
- `DELETE /api/v1/lista-espera/{id}`

Notas:
- Backend es source of truth para FIFO/promoci�n cuando flag activo.
- Frontend no promueve localmente en cancelaci�n cuando API waitlist est� activa.
- No usar `GET /api/v1/lista-espera?claseId=...` en modo API actual.

Siguiente integraci�n recomendada:
- Notificaciones reales / POS-finanzas (seg�n roadmap), o endurecer QA E2E multi-rol.

## QA E2E Auth + Clases + Reservas + Waitlist

Fecha validaci�n: `2026-05-29`.
...
- En modo API waitlist activo no se realiza promoci�n FIFO local en frontend.

Pendientes:
- QA manual visual en navegador con captura de Network por interacci�n UI completa.
...
- Plantilla formal de ejecuci�n QA: [docs/FRONTEND_E2E_QA_RUN_TEMPLATE.md](/D:/Casa_Scarlatta_FrontEnd/docs/FRONTEND_E2E_QA_RUN_TEMPLATE.md)

## Sprint Estabilizaci�n Core (avance)
...
- BUG-003 mitigado en frontend:
  - `reserved_at` ya no se usa como fecha de sesi�n.
  - matching diario de reservas usa ocurrencia real cuando est� disponible.
  - si no hay fecha de sesi�n, UI aplica estado neutral en filtros diarios.
...
- Waitlist API migrada a ocurrencia:
  ...
  - `POST /api/v1/lista-espera` con `occurrence_id`
  - `DELETE /api/v1/lista-espera/{id}`
- BUG-003 queda cerrado end-to-end (pendiente solo QA visual multi-rol de regresi�n).

## Hotfix performance de ocurrencias/waitlist (2026-05-29)
- Se aplic� dedupe in-flight para `GET /clases/{id}/ocurrencias` por llave `classId|from|to`.
- Se agreg� AbortController en cargas por rango para evitar setState tras unmount/cambio r�pido.
- Se elimin� precarga masiva de waitlist en dashboard; waitlist API queda bajo demanda.
- Pr�xima mejora recomendada: TanStack Query + endpoint bulk/BFF.
...
- Compra self-service a�n no implementada.
- El modal muestra estado controlado: `Compra en l�nea a�n no disponible en modo API`.
- El flujo local/mock de compra se mantiene solo como fallback cuando flags API est�n en `false`.

## Estado BUG-004 Core (cerrado)
- Estado: **Cerrado (core)**.
- Cubre: cr�ditos/membres�a persistentes tras login, refresh, reservar y cancelar usando `GET /api/v1/clientes/me/estado-financiero`.
...
- M�tricas de coach en API mode derivan de `agenda.occurrences`.
- Source of truth: `GET /api/v1/coaches/me/agenda?from&to`.
...
- Secci�n �Clases de hoy� en CoachPanel API mode ahora deriva de `agenda.occurrences`.
- Source of truth: `GET /api/v1/coaches/me/agenda?from&to`.
- Filtro por `occurrenceDate` del d�a actual y orden por hora.
...
- �Mis pr�ximas clases� en Dashboard Cliente ya no est� limitado de forma r�gida a 2.
- En API mode, usa reservas reales de `GET /api/v1/reservas/me` con criterio: `confirmada` + hoy/futuras + orden por ocurrencia.
- L�mite visual configurable actual: `4` (`UPCOMING_RESERVATIONS_LIMIT`).
- Se agreg� CTA �Ver todas� hacia �Mis clases� con filtro `confirmada`.
- BUG-006 permanece como siguiente paso para paginaci�n/listados globales.

## Estado BUG-006A/B (mitigaci�n frontend aplicada)
- Se a�adieron controles de paginaci�n visual en listados cr�ticos ya cargados en memoria:
...
- BUG-006C queda pendiente para paginaci�n backend real (`page/page_size/total/items`).

## Estado BUG-006C (frontend incremental)
- Se agreg� compatibilidad de frontend con contrato paginado backend (`page/page_size/total/items`) sin romper legacy.
- Integraciones activas:
  - Admin clases (vista lista, filtro `Todas`) con paginaci�n backend.
  - Cliente movimientos de cr�dito desde endpoint paginado dedicado.
- Se mantiene transici�n segura: respuestas array legacy siguen soportadas.
- Pendiente siguiente: migrar "Mis clases" cliente a paginaci�n backend conservando UX semanal.

## Estado BUG-006C (cierre frontend alcance actual)
- Se cerr� migraci�n paginada de �Mis clases� cliente en API mode usando `reservas/me` paginado con `status/from/to`.
- Se conserva UX semanal/filtro y refetch tras cancelaci�n.
- �Mis pr�ximas clases� permanece estable y separado de este flujo.
- Legacy/fallback mock se mantiene para flags false.
- Pr�ximo paso: paginaci�n backend en listados admin globales cuando backend exponga contratos.

## Cierre pre-BUG-009
Core stabilization listo para QA final pre-Mercado Pago.
Siguiente m�dulo recomendado: BUG-009 (integraci�n Mercado Pago).
...
- Landing usa cat�logo backend real `GET /api/v1/memberships/packages` en API mode.
- Compra desde landing no procesa pago ah�: guarda intenci�n y lleva a login o dashboard pagos.
...
- P�blico ve cat�logo/clases; acciones de reservar siguen bajo auth de cliente.
...
Frontend MVP uses TanStack Query for server state. Zustand queda para fallback legacy y UI local. Lecturas con `useQuery`, mutaciones con `useMutation`, refetch con invalidate tras �xito. No usar `page_size=1000`.

## Integraci�n POS API (Feature Flag)
...
- Venta exitosa abre modal con ticket, PDF y link p�blico para WhatsApp Web.
- `public_ticket_url` es la URL recomendada para compartir.
- `POS` no usa Mercado Pago.
- `finanzasService` queda solo legacy/demo y fallback legacy se conserva cuando flags API est�n en `false`.

## Integraci�n Finanzas Admin (API mode)
...
- M�tricas visibles: ventas, gastos, neto, corte, m�todos de pago, gastos recientes, ventas recientes e historial de cortes.
...
## Integraci�n Reportes Admin (API mode)
