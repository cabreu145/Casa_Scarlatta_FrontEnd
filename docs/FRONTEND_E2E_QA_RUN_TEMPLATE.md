...
- [ ] Admin > Clases (vista lista): validar paginaci�n visual, filtros API (`search`, `discipline`, `status`, `coach_id`) y navegabilidad por p�ginas.
- [ ] Admin > Clases (nuevo): validar que activa en UI se env�a como programada, coach_id es num�rico backend y fecha espec�fica / programar publicaci�n quedan bloqueados en API mode.
