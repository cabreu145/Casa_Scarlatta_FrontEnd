...
| Coaches | `src/pages/admin/sections/CoachesSection.jsx`, flujo en `AdminPanel.jsx` | `coachesStore` como fallback | `coachesApiService`, `coachApiPayload` | **API-first** | listar, crear, editar, eliminar, activar/desactivar, upload avatar, sección pública | `GET/POST/PUT/PATCH/DELETE /api/v1/coaches`, `POST /api/v1/coaches/{id}/avatar`, `GET /api/v1/coaches/public`, opcional `GET /api/v1/coaches/{id}/agenda` | Medio: fallback legacy aún presente | **Admin-1** |
