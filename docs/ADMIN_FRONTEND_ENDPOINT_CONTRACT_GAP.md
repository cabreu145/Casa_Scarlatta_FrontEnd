...
- **Mock/local dominante**: `Dashboard admin`, `Usuarios/clientes`, `Paquetes/membres�as`, `POS/productos/ventas`, `Transacciones/pagos`, `Gastos`, `Cortes`, `Reportes`, `Configuraci�n`.
...
| Coaches | API-first | listo | **P0** | s� | no |
...
- `GET /api/v1/coaches/public` ya es fuente p�blica para landing/Nosotros.
- `POST /api/v1/coaches` requiere `password` en create; update no lo env�a.
